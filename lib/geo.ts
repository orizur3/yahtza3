...
export type VehicleStatus = 'בדרך' | 'הגיע' | 'חזר' | 'תקוע' | 'בהמתנה'
export type PersonnelRole = 'מפקד' | 'מהנדס' | 'מטפל בכיר' | 'חובש' | 'מחלץ'

export const VEHICLE_STATUS_CFG: Record<VehicleStatus, {color:string;bg:string;label:string}> = {
  'בדרך':    { color:'#0c447c', bg:'#e6f1fb', label:'בדרך ליעד' },
  'הגיע':    { color:'#085041', bg:'#e1f5ee', label:'הגיע ליעד' },
  'חזר':     { color:'#27500a', bg:'#eaf3de', label:'בדרך חזרה' },
  'תקוע':    { color:'#633806', bg:'#faeeda', label:'תקוע' },
  'בהמתנה':  { color:'#5f5e5a', bg:'#f1efe8', label:'בהמתנה' },
}

export const ALL_VEHICLE_STATUSES: VehicleStatus[] = ['בדרך','הגיע','חזר','תקוע','בהמתנה']
export const ALL_PERSONNEL_ROLES: PersonnelRole[] = ['מפקד','מהנדס','מטפל בכיר','חובש','מחלץ']
