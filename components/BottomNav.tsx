...
export default function BottomNav({ onNewReport, onSummary, onNewShift }: {
  ...
  const TOOLTIPS: Record<string,string> = {
    '/': 'מסך ראשי — טבלת האירועים והמפה',
    '/personnel': 'ניהול כוח אדם ורכבים',
    '/log': 'יומן אירועים יומי',
    '/dashboard': 'דשבורד פיקוד — סטטיסטיקות',
  }
  const navBtn = (path: string, icon: React.ReactNode, label: string, onClick?: () => void) => (
    <button
      onClick={() => onClick ? onClick() : router.push(path)}
      title={TOOLTIPS[path] || label}
      aria-label={TOOLTIPS[path] || label}
      style={{
    ...
    >
      {icon}
      <span style={{ fontSize: 11, fontWeight: isActive(path) ? 700 : 400, color: isActive(path) ? '#e24b4a' : '#64748b', whiteSpace: 'nowrap' }}>{label}</span>
    </button>
  )
      ...
      <div style={{
        ...
        <div style={{ flex: 1, display: 'flex', justifyContent: 'center', alignItems: 'flex-end', paddingBottom: 2 }}>
          <button onClick={onNewReport} style={{ background: 'none', border: 'none', cursor: 'pointer', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3 }}>
            ...
            <span style={{ fontSize: 11, color: '#64748b' }}>אירוע חדש</span>
          </button>
        </div>
        ...
        >
          ...
          <span style={{ fontSize: 11, color: '#64748b', whiteSpace: 'nowrap' }}>סיכום יומי</span>
        </button>
