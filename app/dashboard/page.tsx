...
const KPI_ICONS: Record<string,string> = {
  'פתוחים':'🔴','סגורים':'✅','סה"כ':'📋','דחופים':'🚨','עם נפגעים':'🩺',
  'ללא כוח':'⚠️','מעל 30 דק':'⏱','גיאוגרפי':'📍','נפגעים':'🩺','מטחים':'🗂'
}
function StatCard({ n, label, color, sub }: { n: number|string; label: string; color: string; sub?: string }) {
  const icon = Object.entries(KPI_ICONS).find(([k]) => label.includes(k))?.[1] || '📊'
  return (
    <div style={{ background: '#1e293b', borderRadius: 14, padding: '18px 12px', minWidth: 110, flex: 1, textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4, border: '1px solid #334155' }}>
      <div style={{ fontSize: 26 }}>{icon}</div>
      <div style={{ fontSize: 34, fontWeight: 800, color, lineHeight: 1 }}>{n}</div>
      <div style={{ fontSize: 13, color: '#94a3b8', marginTop: 3, fontWeight: 500 }}>{label}</div>
...
export default function DashboardPage() {
  ...
  return (
    <div style={{ direction: 'rtl', minHeight: '100vh', paddingRight: 200, paddingBottom: 80, background: '#0a0f1a', fontFamily: 'Arial, sans-serif' }}>
      <div style={{ background: '#0f172a', padding: '10px 22px', display: 'grid', gridTemplateColumns: '1fr auto 1fr', alignItems: 'center', borderBottom: '1px solid #1e293b' }}>
        <div style={{ display: 'flex', gap: 8 }}>
          <button onClick={() => router.push('/')} title='חזרה למסך הראשי' style={{ background: '#1e293b', color: '#94a3b8', border: '1px solid #334155', borderRadius: 8, padding: '7px 14px', fontSize: 12, cursor: 'pointer' }}>→ טבלה</button>
