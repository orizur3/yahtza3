...
export default function Page() {
  ...
  return (
    <div style={{direction:'rtl',minHeight:'100vh',background:'#f3f4f6',fontFamily:'Arial,sans-serif',paddingBottom:80}}>
      ...
      <div style={{background:'#0f172a',padding:'10px 22px',display:'grid',gridTemplateColumns:'1fr auto 1fr',alignItems:'center'}}>
        <div style={{display:'flex',gap:6,alignItems:'center',flexWrap:'wrap'}}>
          {isHQ&&<button onClick={()=>setShowShiftModal(true)} title='פתח מטח עבודה חדש — הטבלה תתרוקן' style={{background:'#1e293b',color:'#94a3b8',border:'1px solid #334155',borderRadius:8,padding:'7px 12px',fontSize:12,cursor:'pointer'}}>{currentShift?'🔄 מטח חדש':'➕ מטח'}</button>}
          {isHQ&&<button onClick={()=>router.push('/shifts')} title='צפה בארכיון המטחים הקודמים' style={{background:'#1e293b',color:'#94a3b8',border:'1px solid #334155',borderRadius:8,padding:'7px 12px',fontSize:12,cursor:'pointer'}}>🗂 מטחים קודמים</button>}
          <button onClick={()=>exportToCSV(filtered)} title='ייצוא הטבלה הנוכחית לקובץ Excel' style={{background:'#14532d',color:'#86efac',border:'1px solid #166534',borderRadius:8,padding:'7px 12px',fontSize:12,cursor:'pointer'}}>📊 Excel</button>
          {isHQ&&<button onClick={()=>router.push('/dashboard')} title='דשבורד פיקוד — סטטיסטיקות וניתוח' style={{background:'#1e293b',color:'#fbbf24',border:'1px solid #92400e',borderRadius:8,padding:'7px 12px',fontSize:13,cursor:'pointer'}}>📊 דשבורד פיקוד</button>}
          ...
          <button onClick={handleLogout} style={{background:'none',color:'#94a3b8',border:'1px solid #334155',borderRadius:8,padding:'7px 10px',fontSize:13,cursor:'pointer'}}>יציאה</button>
        </div>
              ...
              <table style={{width:'100%',borderCollapse:'collapse',minWidth:isHQ?1200:1100}}>
                ...
                <tbody>
                  {filtered.map(r=>{
                    ...
                    const rowBg=isSelected?'#eff6ff':r.priority==='דחוף'&&!CLOSED_STATUSES.includes(r.status)?'#fff5f5':focusReport?.id===r.id?'#f0f9ff':isGeoRelated?'#fffdf0':'transparent'
                    ...
                    return(
                      <tr key={r.id} style={{background:rowBg,borderTop}}>
