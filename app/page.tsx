  ...
  return (
    <div onClick={onClose} style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.5)', zIndex:2000, display:'flex', alignItems:'center', justifyContent:'center', padding:20 }}>
      <div onClick={e=>e.stopPropagation()} style={{ background:'#fff', borderRadius:14, padding:24, minWidth:360, direction:'rtl' }}>
        ...
        <p style={{ fontSize:13, color:'#6b7280', marginBottom:16 }}>{currentShift?`מטח "${currentShift.name}" ייסגר. רק אירועים פתוחים יועברו.`:'פתיחת המטח הראשון'}</p>
    ...
    <div onClick={onClose} style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.5)', zIndex:2000, display:'flex', alignItems:'center', justifyContent:'center', padding:20 }}>
      <div onClick={e=>e.stopPropagation()} style={{ background:'#fff', borderRadius:14, padding:22, width:'100%', maxWidth:500, maxHeight:'80vh', overflowY:'auto', direction:'rtl' }}>
        ...
        {loading ? <div style={{ textAlign:'center', color:'#9ca3af', padding:20 }}>טוען...</div>
          ...
          : entries.map(e => (
            <div key={e.id} style={{ borderBottom:'1px solid #f0f0f0', padding:'10px 0' }}>
              <div style={{ display:'flex', justifyContent:'space-between', marginBottom:4 }}>
                <div style={{ display:'flex', gap:8, alignItems:'center' }}>
  ...
  const initMap = useCallback(()=>{
    ...
    withCoords.forEach(r=>{
      ...
      const marker = new window.google.maps.Marker({
        ...
        icon:{path:window.google.maps.SymbolPath.CIRCLE,scale:isActive?14:10,fillColor:mapColor,fillOpacity:1,strokeColor:'#fff',strokeWeight:isActive?3:2},
...
function exportDailySummary(reports:Report[],shifts:Shift[]) {
  ...
  <style>body{font-family:Arial;direction:rtl;padding:30px;font-size:13px}h1{font-size:22px}h2{font-size:16px;margin-top:24px;border-bottom:2px solid #e5e7eb;padding-bottom:6px}.stat{display:inline-block;background:#f9fafb;border:1px solid #e5e7eb;border-radius:8px;padding:12px 20px;margin:6px;text-align:center}.stat-n{font-size:28px;font-weight:800;color:#1d4ed8}.stat-l{font-size:12px;color:#6b7280}table{width:100%;border-collapse:collapse;margin-top:12px}th{background:#0f172a;color:white;padding:8px;text-align:right;font-size:12px}td{padding:7px 8px;border-bottom:1px solid #e5e7eb;font-size:12px}</style>
  </head><body><h1>סיכום יומי — ${today}</h1>
  <div><div class="stat"><div class="stat-n">${reports.filter(r=>!r.is_merged).length}</div><div class="stat-l">סה"כ</div></div>
  <div class="stat"><div class="stat-n" style="color:#e24b4a">${open.length}</div><div class="stat-l">פתוחים</div></div>
  <div class="stat"><div class="stat-n" style="color:#1d9e75">${reports.filter(r=>CLOSED_STATUSES.includes(r.status)).length}</div><div class="stat-l">סגורים</div></div>
  ${ALL_COMPANIES.map(c=>`<div class="stat"><div class="stat-n" style="color:${COMPANY_CFG[c].color}">${reports.filter(r=>r.assigned_company===c&&!r.is_merged).length}</div><div class="stat-l">${COMPANY_CFG[c].label}</div></div>`).join('')}
  </div>
  <h2>אירועים פתוחים</h2>
  <table><thead><tr><th>פלוגה</th><th>עיר</th><th>רחוב</th><th>עדיפות</th><th>סטטוס</th><th>כוח</th><th>שעה</th></tr></thead><tbody>
    ${open.map(r=>`<tr><td>${r.assigned_company||'—'}</td><td>${r.city}</td><td>${r.street}</td><td>${r.priority||'רגיל'}</td><td>${r.status}</td><td>${r.assigned_unit||'—'}</td><td>${new Date(r.report_time).toLocaleTimeString('he-IL',{hour:'2-digit',minute:'2-digit'})}</td></tr>`).join('')}
  ...
  return (
    <div style={{direction:'rtl'}}>
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'10px 16px'}}>
        ...
        <div><label style={{display:'block',fontSize:12,fontWeight:700,marginBottom:4,color:'#374151'}}>כוח מוקצה</label><input style={inp} value={form.assigned_unit} onChange={handleUnit} placeholder="ניידת 3..."/></div>
...
export default function Page() {
  ...
  const addLog=useCallback(async(content:string,reportId?:string)=>{
    await supabase.from('event_log').insert({entry_type:'auto',content,created_by:'מערכת',related_report_id:reportId||null,shift_id:currentShift?.id||null,reported_by_role:userRole})
  },[currentShift])
  ...
  const handleMerge=async(keepId:string,mergeId:string)=>{
    ...
    await supabase.from('reports').update({street:mergedStreet,location_text:`${mergedStreet}, ${keep.city}`,casualties:(keep.casualties||0)+(merge.casualties||0),report_content:[keep.report_content,merge.report_content].filter(Boolean).join(' | '),merged_with:[...(keep.merged_with||[]),mergeId]}).eq('id',keepId)
  ...
  const handleStartShift=async(name:string)=>{
    ...
    if(newShift){
      // Transfer only NON-closed open reports
      const openReports=reports.filter(r=>!CLOSED_STATUSES.includes(r.status as any)&&!r.is_merged)
      for(const r of openReports) await supabase.from('reports').update({shift_id:(newShift as Shift).id}).eq('id',r.id)
  ...
  const sortedReports=[...roleFilteredReports].filter(r=>!r.is_merged).sort((a,b)=>{
  ...
  const openCount=roleFilteredReports.filter(r=>!CLOSED_STATUSES.includes(r.status)&&!r.is_merged).length
  const criticalCount=roleFilteredReports.filter(r=>r.priority==='דחוף'&&!CLOSED_STATUSES.includes(r.status)&&!r.is_merged).length
    ...
    <div style={{direction:'rtl',minHeight:'100vh',background:'#f3f4f6',fontFamily:'Arial,sans-serif'}}>
      ...
      <div style={{background:'#0f172a',padding:'10px 22px',display:'grid',gridTemplateColumns:'1fr auto 1fr',alignItems:'center'}}>
        <div style={{display:'flex',gap:6,alignItems:'center',flexWrap:'wrap'}}>
          ...
          <button onClick={()=>router.push('/shifts')} style={{background:'#1e293b',color:'#94a3b8',border:'1px solid #334155',borderRadius:8,padding:'7px 12px',fontSize:12,cursor:'pointer'}}>🗂 מטחים</button>
        ...
        <div style={{display:'flex',justifyContent:'flex-end',gap:8,alignItems:'center'}}>
          {isHQ&&<button onClick={()=>setShowShiftModal(true)} style={{background:'#1e293b',color:'#94a3b8',border:'1px solid #334155',borderRadius:8,padding:'7px 12px',fontSize:12,cursor:'pointer'}}>{currentShift?'🔄 מטח חדש':'➕ פתח מטח'}</button>}
          {criticalCount>0&&<div style={{background:'#450a0a',borderRadius:8,padding:'6px 12px',textAlign:'center'}}><div style={{fontSize:20,fontWeight:800,color:'#f87171'}}>{criticalCount}</div><div style={{fontSize:10,color:'#94a3b8'}}>דחופים</div></div>}
          <div style={{background:'#1e293b',borderRadius:8,padding:'6px 12px',textAlign:'center'}}><div style={{fontSize:20,fontWeight:800,color:'#60a5fa'}}>{openCount}</div><div style={{fontSize:10,color:'#94a3b8'}}>פתוחים</div></div>
      ...
      <div style={{padding:16}}>
              ...
              <table style={{width:'100%',borderCollapse:'collapse',minWidth:isHQ?1200:1100}}>
                ...
                <tbody>
                  {filtered.map(r=>{
                    ...
                    const rowBg=isSelected?'#eff6ff':r.priority==='דחוף'&&!CLOSED_STATUSES.includes(r.status)?'#fff5f5':focusReport?.id===r.id?'#f0f9ff':'transparent'
                    ...
                    return(
                      <tr key={r.id} style={{background:rowBg}}>
                        ...
                        <td style={cell} onClick={()=>focusOnMap(r)}>
                          <b style={{borderRight:companyCfg?`3px solid ${companyCfg.mapColor}`:'none',paddingRight:companyCfg?6:0}}>{r.city}</b>
                        </td>
        ...
        <div ref={mapSectionRef}>
          <InteractiveMap reports={roleFilteredReports} focusReport={focusReport} userRole={userRole}/>
        </div>
      </div>
    </div>
  )
