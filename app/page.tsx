...
function exportDailySummary(reports:Report[],shifts:Shift[]) {
  const today=new Date().toLocaleDateString('he-IL')
  const active=reports.filter(r=>!r.is_merged)
  ...
  sortedReports.forEach(r => {
    ...
    if (r.geo_status !== 'ללא קשר' && r.geo_related_reports?.length) {
      r.geo_related_reports.forEach(relId => {
        const rel = sortedReports.find(x => x.id === relId && !addedIds.has(x.id))
    ...
    <div style={{direction:'rtl',minHeight:'100vh',background:'#f3f4f6',fontFamily:'Arial,sans-serif',paddingBottom:80}}>
      ...
      <div style={{background:'#0f172a',padding:'10px 22px',display:'grid',gridTemplateColumns:'1fr auto 1fr',alignItems:'center'}}>
        <div style={{display:'flex',gap:6,alignItems:'center',flexWrap:'wrap'}}>
          {isHQ&&<button onClick={()=>setShowShiftModal(true)} style={{background:'#1e293b',color:'#94a3b8',border:'1px solid #334155',borderRadius:8,padding:'7px 12px',fontSize:12,cursor:'pointer'}}>{currentShift?'🔄 מטח חדש':'➕ מטח'}</button>}
          {isHQ&&<button onClick={()=>router.push('/shifts')} style={{background:'#1e293b',color:'#94a3b8',border:'1px solid #334155',borderRadius:8,padding:'7px 12px',fontSize:12,cursor:'pointer'}}>🗂 מטחים קודמים</button>}
      ...
      <BottomNav onNewReport={()=>{setShowForm(true);setEditReport(null);window.scrollTo(0,0)}} onSummary={()=>exportDailySummary(reports,shifts)} onNewShift={()=>setShowShiftModal(true)}/>
    </div>
  )
}
